...
plt.plot(
    num_trees, oob_score, color="red", linestyle=":", label="random_forest_oob_score"
)
...
rfc = RFC(n_estimators=100, max_features="sqrt", oob_score=True, random_state=0)
rfc.fit(x, y)
...
with tqdm(M) as pbar:
    for m in pbar:
        ...
        # 随机森林算法
        rfc = RFC(n_estimators=m, max_depth=1, min_samples_leaf=1, random_state=0)
        rfc.fit(X_train, y_train)
        rf_score.append(rfc.score(X_test, y_test))
        # 离散 AdaBoost，SAMME是分步加性模型（stepwise additive model）的缩写
        dsc_adaboost = AdaBoostClassifier(
            estimator=stump, n_estimators=m, algorithm="SAMME", random_state=0
        )
        dsc_adaboost.fit(X_train, y_train)
        dsc_ada_score.append(dsc_adaboost.score(X_test, y_test))
        # 实 AdaBoost，SAMME.R表示弱分类器输出实数
        real_adaboost = AdaBoostClassifier(
            estimator=stump, n_estimators=m, algorithm="SAMME.R", random_state=0
        )
...
rfr = RandomForestRegressor(
    n_estimators=100, max_depth=1, max_features="sqrt", random_state=0
)
...
stkr_pt = StackingRegressor(
    estimators=[("knn", knnr), ("ln", lnr), ("rf", rfr)], passthrough=True
)
