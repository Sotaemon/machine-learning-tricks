...
plt.scatter(
    features[pos_index, 0], features[pos_index, 1], marker="o", color="coral", s=10
)
plt.scatter(
    features[neg_index, 0], features[neg_index, 1], marker="x", color="blue", s=10
)
plt.xlabel("X1 axis")
plt.ylabel("X2 axis")
plt.show()
...
def gradient_descent(num_epochs, learning_rate, weight_decay):
    ...
    for i in range(num_epochs):
        ...
        train_loss = (
            -train_labels.T @ np.log(pred)
            - (1 - train_labels).T @ np.log(1 - pred)
            + weight_decay * np.linalg.norm(theta) ** 2 / 2
        )
        train_losses.append(train_loss / len(train_set))
        test_pred = logistic(test_set @ theta)
        test_loss = -test_labels.T @ np.log(test_pred) - (1 - test_labels).T @ np.log(
            1 - test_pred
        )
...
theta, train_losses, test_losses, train_acc, test_acc, train_auc, test_auc = (
    gradient_descent(num_epochs, learning_rate, weight_decay)
)
